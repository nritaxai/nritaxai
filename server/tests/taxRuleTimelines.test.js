...
test("getTaxRuleTimelinesForQuery returns dated periods for LTCG questions", () => {
  const timelines = getTaxRuleTimelinesForQuery("Explain Section 112A LTCG on listed equity and equity mutual funds after the new law change.");
  assert.equal(timelines.length, 1);
  assert.equal(timelines[0].ruleName, "Section 112A LTCG for listed equity, equity mutual funds, and business trusts");
  assert.equal(timelines[0].periods[0].label, "Before 23 Jul 2024");
  assert.equal(timelines[0].periods[1].label, "On/after 23 Jul 2024");
...
test("resolveApplicablePeriod selects the pre-change LTCG rule for earlier dates", () => {
  ...
  const applicable = resolveApplicablePeriod(timeline.periods, "2024-07-15");
...
test("appendTimelineToAnswer injects date-wise tax rule sections into markdown", () => {
  ...
  assert.match(reply, /#### Section 112A LTCG for listed equity, equity mutual funds, and business trusts/);
  assert.match(reply, /The applicable rule depends on the transaction date\./);
  assert.match(reply, /Before 23 Jul 2024: 10% tax, ₹1,00,000 exemption/);
  assert.match(reply, /On\/after 23 Jul 2024: 12.5% tax, ₹1,25,000 exemption/);
});

test("calculateCapitalGainsTax uses dated LTCG rule when sale date is before 23 Jul 2024", () => {
  const req = {
    body: {
  ...
  assert.equal(res.body.result.applicableRule.label, "Before 23 Jul 2024");
