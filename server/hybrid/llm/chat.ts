...
export class HybridChatClient {
  ...
  async generateWithOllama(systemPrompt: string, prompt: string): Promise<string> {
    try {
      const result = await this.providerClient.generateWithOllama(systemPrompt, prompt);
      return result.content;
    } catch (error) {
      throw new Error(
        `Ollama generation failed: ${error instanceof Error ? error.message : "Unknown Ollama error"}`
      );
    }
  }
