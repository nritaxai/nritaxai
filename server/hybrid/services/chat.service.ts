...
export class ChatService {
  ...
  private async retrieve(message: string): Promise<RetrievalState> {
    try {
      ...
      return await this.retriever.search(embeddingResult.embedding, undefined, message);
    } catch {
  ...
  private async runOllamaDirect(
    message: string,
    contextText: string,
    citationsText: string
  ): Promise<ChatExecution> {
    const answer = await this.chatClient.generateWithOllama(
      HYBRID_SYSTEM_PROMPT,
      `Use the retrieved context first and answer concisely.\n\nContext:\n${contextText || "No context retrieved."}\n\nReferences:\n${citationsText || "No references available."}\n\nQuestion:\n${message}`
    );

    return {
      answer,
      verificationApplied: false,
      providerTrail: ["ollama_direct"],
    };
  }
  ...
  async handleChat(input: HybridChatRequest): Promise<HybridChatResponse> {
    ...
    try {
      ...
      } else if (routing.mode === "OLLAMA_DIRECT") {
        execution = await this.runOllamaDirect(message, builtContext.contextText, citationsText);
