...
const buildOllamaMessages = (messages = [], systemPrompt = "") => {
...
export const callOllama = async (
  ...
  const requestMessages = buildOllamaMessages(messages, systemPrompt);
  if (!requestMessages.length) {
    throw new Error("Ollama requires at least one message");
  }

  const { controller, timeoutId } = createTimeoutSignal(OLLAMA_TIMEOUT_MS);
  ...
  try {
    const response = await fetch(`${OLLAMA_BASE_URL}/api/chat`, {
      ...
      body: JSON.stringify({
        model: String(preferredModel || OLLAMA_MODEL || "").trim(),
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw createHttpError("Ollama", response.status, errorText);
    }

    const data = await response.json();
    const answer = normalizeMessageContent(data?.message?.content);
    if (!answer) {
    ...
    return {
      response: answer,
      provider: "ollama",
      model: String(preferredModel || OLLAMA_MODEL),
      finishReason: String(data?.done_reason || "stop"),
    };
  } catch (error) {
    console.error("[AI] Ollama call failed:", error.message);
    throw error;
  } finally {
    clearTimeout(timeoutId);
  }
